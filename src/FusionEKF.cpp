...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    cout << "EKF: " << endl;
    ekf_.x_ = VectorXd(4);
    ekf_.x_ << 0, 0, 5 , 0; //this value is import for the RMSE, need to fine tune
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::LASER) {
      //Initialize location and zero velocity.
  ...
  float dt = (float)dt_ll/ 1000000.0;	//dt - expressed in seconds
  previous_timestamp_ = measurement_pack.timestamp_;
